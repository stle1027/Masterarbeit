...
class OpenSetDetectorWithExamples(nn.Module):
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]], dinov2_model):
        ...
        if self.training:
            ...
            if self.use_one_shot:
                ...
                # class_weights are the global prototypes
        ...
        with torch.no_grad():
            ...
            with autocast(enabled=True):
                # extract DINOv2 VIT features (all layers)
                dinov2_all_layers = dinov2_model.get_intermediate_layers(images.tensor.to(self.device), 
                        return_class_token=True, reshape=True,n=12)
                # extract the features from last layer from dinov2_all_layers
                dinov2_patch_tokens_last_layer = dinov2_all_layers[11][0][0]
                dinov2_patch_tokens_last_layer = dinov2_patch_tokens_last_layer.unsqueeze(0)
                # patch tokens (last layer DINOv2 ViT features are used for standard DE-ViT with global prototype)
                patch_tokens = dinov2_patch_tokens_last_layer
        ...
        if self.training or self.use_one_shot: 
            ...
            # make region proposals to boxes
            boxes = proposals[0].proposal_boxes.tensor
            # add vector with zeros in fromt of box tensor. -> makes shape (1000,5) out of (1000,4) (highly time consuming process...)
            rois = torch.cat([torch.full((len(boxes), 1), fill_value=0).to(self.device) , 
                            boxes], dim=1)
        # extract ViT features which are inside the roi AND scale each roi_feature box to 7x7 spatial and 768 dimension
        ...
        if (self.training and (not self.only_train_mask)) or (not self.training):
            ...
            # matrix multiplication between ViT roi features (scaled boxes) and global prototypes (feature subspace projection???)
            ...
            if sample_class_enabled:
                ...
                # get initial similarity score between proposal boxes and all class prototypes
                init_scores = F.normalize(roi_features.flatten(2).mean(2), dim=1) @ class_weights.T
                # get the k class indices that are most similar to the proposal box 
                topk_class_indices = torch.topk(init_scores, class_topk, dim=1).indices
                # get the k class indices that are most similar to the proposal box, used for fine-grained differentionn between similar objects later
                topk_3_class_indices = torch.topk(init_scores, 3, dim=1).indices
                ...
                if self.training:
                    ...
                    # class_indice is list with class indice that is most similar for each region proposal
                    class_indices = topk_class_indices
                # sort the topk indices based on similarity to proposal for each proposal (only relevant if topk > 1)
                class_indices = torch.sort(class_indices, dim=1).values
            else:
                num_active_classes = num_classes
            # begin: retrieve features from feats (roi_features @ global prototypes) that are not the features of the topk classes
            other_classes = [] 
            if sample_class_enabled:
            ...
            # end: retrieve features from feats (roi_features @ global prototypes) that are not the features of the topk classes
            # sort features of other classes at dimension of other classes
            other_classes, _ = torch.sort(other_classes, dim=-1)
            # interpolate features of the "number_other_classes" to 128 other classes
            other_classes = interpolate(other_classes, self.T, mode='linear') # (Nxclasses) x spatial x T
            # put other_classes features in learned linear layer
            ...
            # put features from topk after positional embedding in linear layer
            ...
            # multiply roi features with background prototypes
            bg_feats = roi_features.transpose(-2, -1) @ self.bg_tokens.T # N x spatial x back
            # put bg feats in linear layer
            ...
            # (Nxclasses) x EMB x S x S
            # combine features from topk, not topk and background
            per_cls_input = torch.cat([intra_dist_emb, inter_dist_emb, bg_dist_emb_c], dim=1)

            # (Nxclasses) x 1
            # calculate score for each of topk classes for each proposal with propagate net
            ...
            # calculate score for each proposal if it is background with propagate net
            bg_logits = self.bg_cnn(torch.cat([bg_cls_dist_emb, bg_dist_emb], dim=1))
            # combine topk scores and background score in one row for each proposal
        ...
        if (self.training and (not self.only_train_mask)) or (not self.training):
            ...
            # extract ViT features which are inside the expanded roi AND scale each roi_feature box to 20x20 spatial and 768 dimension
            aroi_feats = self.reg_roi_align(patch_tokens, aug_rois) # N x C x K x K
            aroi_feats = aroi_feats.flatten(2) # N x C x K2
            
            # multiply expanded roi features with background prototypes
            bg_aroi_feats = aroi_feats.transpose(-2, -1) @ self.bg_tokens.T # N x K2 x back
            # put expanded background features in linear layer
            bg_aroi_emb = self.reg_bg_dist_emb(bg_aroi_feats)  # N x K2 x T
            # multiply expanded roi features with global prototypes
            ...
            if self.training:
                ...
                # (NxK2) x class x emb
                # retrieve only expanded roi features from topk for each proposal
                ...
                # apply positional embedding on expanded roi features from topk 
                fg_aroi_emb = distance_embed(fg_aroi_dist_feats.flatten(0, 1), num_pos_feats=self.Tpos_emb)
                # time relevant, takes 250 ms !!!
                # put expanded roi features from topk in linear layer
                fg_aroi_emb = self.reg_intra_dist_emb(fg_aroi_emb)
                ...
                # combine expanded background roi features and expanded global prototype roi features
                aroi_emb = torch.cat([fg_aroi_emb, bg_aroi_emb], dim=1)
                # (Nxclass) x K x K
                # create mask with zeros for each position of the feature map
            ...
            for i, (rp, rp_out) in enumerate([
                ...
                masks.insert(0, pred_mask_logits.sigmoid())
                # calculate relative bounding box coordinates for each proposal with spatial integral from propagated features (1000)
                ...
                if self.training:
                    ...
                    try:
                        ...
                        pass
            # transform relative bounding box coordinates to real bounding box coordinates
            pred_abs_boxes = region_coord_2_abs_coord(aug_rois[:, 1:], pred_region_coords, self.reg_roialign_size)
            # calculate deltas with learnable box2box transform using pred_abs_boxes and original rois
        ...
        if self.training:
            ...
            # put scores [topk score, background score] between 0 and 1 with softmax
            scores = F.softmax(logits, dim=-1)
            # output is score for topk classes for each proposal
            output = {'scores': scores[:, :-1] }
            # apply deltas on original proposal boxes
            ...
            ######################### LEON wert zwischn 0 und 1
            # make combined score with score for topk and objectness score of proposal
            if self.mult_rpn_score:
                rpn_scores = [x.objectness_logits for x in proposals][0]
                rpn_scores[rpn_scores < 0] = 0
                
                # Min-Max-Skalierung der rpn_scores
                min_rpn_score = rpn_scores.min()
                max_rpn_score = rpn_scores.max()
                
                if max_rpn_score > min_rpn_score:  # Vermeidung von Division durch Null
                    rpn_scores = (rpn_scores - min_rpn_score) / (max_rpn_score - min_rpn_score)
                else:
                    rpn_scores = torch.zeros_like(rpn_scores)  # Falls alle rpn_scores gleich sind
            ...
            filtered_topk3_indices = topk_3_class_indices[kept_indices]
            # relevant time 110 ms
            output['instances'] = results[0]['instances']
            print("HIIIIIIIIIIIIIIIII",results[0]['instances'].shape)
            output["topk3"] = filtered_topk3_indices

            return output, dinov2_all_layers